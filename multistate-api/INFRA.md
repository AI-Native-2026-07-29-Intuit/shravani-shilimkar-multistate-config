...
| W28 | `CfnDeployRole`, `MultistateAppSecurityGroup`, `DbInstance`, `EstimatedChargesAlarm` | Names are pinned deliberately — the GitHub Actions trust policy references `CfnDeployRole`'s name directly, the app SG's name is a stable operational label, `DbInstance`'s identifier is what appears in the RDS console/CLI day to day, and `EstimatedChargesAlarm`'s name (`multistate/estimated-charges-dev`) is the one used to look it up (`aws cloudwatch describe-alarms --alarm-names ...`) and is referenced in the Task 1 cost runbook. A CFN-generated random suffix on any of these would break something a human or another system depends on by name. |
